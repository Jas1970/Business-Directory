...
$slcomp = "select * from state order by stname";
...
if ($numrows < 1)
	...
	print "<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"state.php\"> Click Here </a></td></tr>";
	print "</table>";
	exit;
...
$display_block .= "</FORM>";
	...
	{
//	
	$result=mysql_query("SELECT stid FROM state ORDER BY stid") or die("Query failed");
	
	$query = "select * from state where stid=";
		$flag=TRUE;
	   ...
	   if($rcdcount==0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"state_del.php\"> Click Here </a></td></tr>
			</table>";
			exit;
			}	
	   
	   $srvsid = mysql_result($result,0,'stid');
	   
	   $chksl = "select * from dir_add where dir_stat='$srvsid'";
	   ...
	   if($chknum<>0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"state_del.php\"> Delete Another One</a></td></tr>
			</table>";
			}
			else
	   		{
				$delrcd = "delete from state where stid= '$srvsid'";
				if($delnum = mysql_query($delrcd,$abc))
					...
					<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"state_del.php\"> Try Again Please</a></td></tr>
					</table>";
