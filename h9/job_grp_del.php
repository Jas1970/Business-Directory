...
if ($numrows < 1)
	...
	$display_block .= "<tr>
							<td align=\"center\" class=\"org_border_box\" width=\"430px\" colspan=\"2\">Delete From Job Group </td>
					  </tr>";
	$rownum = abs(1);
...
$display_block .= "</FORM>";
	...
	{
//	
	$result=mysql_query("SELECT grp_id FROM job_grp ORDER BY grp_id") or die("Query failed");
	
	$query = "select * from job_grp where grp_id=";
		$flag=TRUE;
	   ...
	   if($rcdcount==0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_grp_del.php\"> Click Here </a></td></tr>
			</table>";
			exit;
			}	
	   
	   $srvsid = mysql_result($result,0,'grp_id');
	   
	   $chksl = "select * from job_main where job_grp='$srvsid'";
	   ...
	   if($chknum<>0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_grp_del.php\"> Delete Another One</a></td></tr>
			</table>";
			}
			else
	   		{
				$delrcd = "delete from job_grp where grp_id= '$srvsid'";
				if($delnum = mysql_query($delrcd,$abc))
					...
					<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_grp_del.php\"> Try Again Please</a></td></tr>
					</table>";
