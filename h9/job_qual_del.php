...
if ($numrows < 1)
	...
	$display_block .= "<tr>
							<td align=\"center\" class=\"org_border_box\" width=\"430px\" colspan=\"2\">Delete From Qualification Master</td>
					  </tr>";
	$rownum = abs(1);

	while ($pinfo = mysql_fetch_array($rcdcomp))
	 	{
   		$srvsid	= $pinfo['jqual_id'];
		$srvsname = $pinfo['jqual_name'];		
		$display_block .= " 
			<tr>
...
$display_block .= "</FORM>";
	...
	{
//	
	$result=mysql_query("SELECT jqual_id FROM job_qual ORDER BY jqual_id") or die("Query failed");
	
	$query = "select * from job_qual where jqual_id=";
		$flag=TRUE;
	   ...
	   if($rcdcount==0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_qual_del.php\"> Click Here </a></td></tr>
			</table>";
			exit;
			}	
	   
	   $srvsid = mysql_result($result,0,'jqual_id');
	   
	   $chksl = "select * from job_main where job_qual='$srvsid'";
	   ...
	   if($chknum<>0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_qual_del.php\"> Delete Another One</a></td></tr>
			</table>";
			}
			else
	   		{
				$delrcd = "delete from job_qual where jqual_id= '$srvsid'";
				if($delnum = mysql_query($delrcd,$abc))
					...
					<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"job_qual_del.php\"> Try Again Please</a></td></tr>
					</table>";
