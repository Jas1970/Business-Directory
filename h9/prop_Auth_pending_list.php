...
$rownum = abs(1) ;
	while ($posts_info = mysql_fetch_array($asorcd))
	 {
 		$cid   = $posts_info['prop_comp_id'];
		$ctype = $posts_info['prop_comp_type'];
		 if($ctype=="1")
		 	{
			...
			$slrcd	= "select * from prop_tmain where prop_comp_id='$cid' and prop_comp_type='1'";
			$rdrcd  = mysql_query($slrcd,$abc);
			$numrcd = mysql_num_rows($rdrcd);
			$display_block .= " 
		 	<tr>
		 		<td width=\"550\">$rownum. <font color=\"#990000\" size=\"2\"> $cname - $city_name ($state_name) --(Dir) $scno ($cid) </font></td> <td width=\"100\">(Records : $numrcd) </td> 
			</tr>";
			}
		else	
			{
			...
			$slrcd	= "select * from prop_tmain where prop_comp_id='$cid' and prop_comp_type='2'";
			$rdrcd  = mysql_query($slrcd,$abc);
			$numrcd = mysql_num_rows($rdrcd);
	       	$display_block .= " 
		 	<tr>
