...
if ($numrows < 1)
	...
	$display_block .= "<tr>
							<td align=\"center\" class=\"org_border_box\" width=\"430px\" colspan=\"2\">Delete From Property Type</td>
					  </tr>";
	$rownum = abs(1);

	while ($pinfo = mysql_fetch_array($rcdcomp))
	 	{
   		$srvsid	= $pinfo['type_id'];
		$srvsname = $pinfo['type_name'];		
		$display_block .= " 
			<tr>
...
$display_block .= "</FORM>";
	...
	{
//	
	$result=mysql_query("SELECT type_id FROM prop_type ORDER BY type_id") or die("Query failed");
	
	$query = "select * from prop_type where type_id=";
		$flag=TRUE;
	   ...
	   if($rcdcount==0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"prop_type_del.php\"> Click Here </a></td></tr>
			</table>";
			exit;
			}	
	   
	   $srvsid = mysql_result($result,0,'type_id');
	   
	   $chksl = "select * from prop_main where prop_type='$srvsid'";
	   $chkrd = mysql_query($chksl,$abc);
	   $chknum = mysql_num_rows($chkrd);
	   if($chknum<>0)
	   		{
			$display_block .= "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
			<tr><td class=\"org_border_t_cell\" align=\"center\">Record Exist in Property  Main Table Name Cant Be Deleted </td</tr>
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"prop_type_del.php\"> Delete Another One</a></td></tr>
			</table>";
			}
			else
	   		{
				$delrcd = "delete from prop_type where type_id= '$srvsid'";
				if($delnum = mysql_query($delrcd,$abc))
					...
					<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"prop_type_del.php\"> Try Again Please</a></td></tr>
					</table>";
