            ...
            <td height="66" align="center" valign="top"> 
		      ...
		      if ($op != "auth") 
				...
				if ($numrows < 1)
					...
					print "<form method=post action=\"0708150364.php?ID=$srvsid&SID=$sid&rand=$adms\">
...
print  "</FORM>";
	...
	$query = "select * from job_tmain where tjob_id=";
	   ...
	   if($rcdcount==0)
			...
			<tr><td class=\"org_border_b_cell\" align=\"center\"><a href=\"0708150364.php?ID=$srvsid&SID=$sid&rand=$adms\"> Click Here </a></td></tr>
			</table>";
			exit;
			}	
        ...
        $result = mysql_query($query) or die("<h4> You are Not Select Record Or Select More Then One,  <br> <a 					
					href=\"0708150364.php?ID=$srvsid&SID=$sid&rand=$adms\"> Try Again Please </a></h4>");
		...
		while($inst = mysql_fetch_array($DirAdvtRd)) 
				...
				$advtbal	= $advtnum-$advtcount;
					...
					if($advtbal==0)
						{
						print "<center><h4>Your Record Limit is Over <br>
										Company Id = $srvsid <br>
							<a href=\"0708150333.php?ID=$srvsid&SID=$sid&rand=$adms\"> Enter After avialable Limit </a></h4></center>" ;
						}
						else
						{
						...
						if (mysql_query($inssql)) 
							...
							print "<center><h4>Record Successfuly Authorised <br>
										Company Id = $srvsid <br>
							<a href=\"0708150364.php?ID=$srvsid&SID=$sid&rand=$adms\"> Authorised Another One </a></h4></center>" ;
