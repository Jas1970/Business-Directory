...
//==
		...
		$srvsid= $_REQUEST['SID'];
		$ssrvsid =  $_REQUEST['SSID'];

		$country  = $_POST['country_dropdown'];
		$stat	  = $_POST['state_dropdown'];
...
if($ssrvsid<>"-1" and $country=="-1" and $stat=="-1" and $district=="-1" and $city=="-1")
	{
	...
	if($pg!=1)
	{
		print("<a href=0708150158.php?SID=$srvsid&SSID=$ssrvsid&country=$country&state=$stat&district=$district&city=$city&pg=".($pg-1)." alt='Page ".($pg-1)."'>Previous</a><<&nbsp;");
	}
	for($i=1;$i<=$pages;$i++)		
	{
		if($pages>1)
			print("<a href=0708150158.php?SID=$srvsid&SSID=$ssrvsid&country=$country&state=$stat&district=$district&city=$city&pg=$i alt='Page $i'><u>$i</u></a>&nbsp;");
	}
	if($pg+1<=$pages)
	{
		print("&nbsp;>><a href=0708150158.php?SID=$srvsid&SSID=$ssrvsid&country=$country&state=$stat&district=$district&city=$city&pg=".($pg+1)." alt='Page ".($pg+1)."'>Next</a>");
	}
	print("</font></td><td align=right width=%15><font face=arial size=2 color=Blue>Page $pg of ".(($pages==0)?1:$pages)."</font></td></tr></table>");
	}
//================================Second Loop =============================	

elseif($ssrvsid<>"-1" and $country<>"-1" and $stat=="-1" and $district=="-1" and $city=="-1")
	{

	$query = "SELECT distinct(srvs_advts.mids) FROM srvs_advts 
			 where srvs_advts.srid = '$ssrvsid' and
			  srvs_advts.srvsid='$srvsid' and
			  srvs_advts.country='$country'order by srvs_advts.sidx";
				...
				{	 
				 ...
				 if($rdnum<>"0")
...

					{
						...
						$srname = "$srvs_name : " . "($subsrvs_name) 2";
...
elseif($ssrvsid<>"-1" and $country<>"-1" and $stat<>"-1" and $district=="-1" and $city=="-1")
	...
	$query = "SELECT distinct(srvs_advts.mids) FROM srvs_advts 
			 where srvs_advts.srid = '$ssrvsid' and
			  srvs_advts.srvsid='$srvsid' and
			  srvs_advts.country='$country' and
			  srvs_advts.state='$stat' order by srvs_advts.sidx";
				...
				{	 
				 ...
				 if($rdnum<>"0")
...

					{
						...
						$srname = "$srvs_name : " . "($subsrvs_name) 3";
...
elseif($ssrvsid<>"-1" and $country<>"-1" and $stat<>"-1" and $district<>"-1" and $city=="-1")
	...
	$query = "SELECT distinct(srvs_advts.mids) FROM srvs_advts 
			 where srvs_advts.srid = '$ssrvsid' and
			  srvs_advts.srvsid='$srvsid' and
			  srvs_advts.country='$country' and
			  srvs_advts.state='$stat' and 
			  srvs_advts.district= '$district' 
			  order by srvs_advts.sidx";
				...
				{	 
				 ...
				 if($rdnum<>"0")
...

					{
						...
						$srname = "$srvs_name : " . "($subsrvs_name) 4";
...
elseif($ssrvsid<>"-1" and $country<>"-1" and $stat<>"-1" and $district<>"-1" and $city<>"-1")
	...
	$query = "SELECT distinct(srvs_advts.mids) FROM srvs_advts 
			 where srvs_advts.srid = '$ssrvsid' and
			  ...
			  srvs_advts.country='$country' and
			  srvs_advts.state='$stat' and 
			  srvs_advts.district= '$district' and
			  srvs_advts.city= '$city' order by srvs_advts.sidx";
				...
				{	 
				 ...
				 if($rdnum<>"0")
...

					{
						...
						$srname = "$srvs_name : " . "($subsrvs_name) 5";
	...
	else
	{
			...
			print "<br> <table align=\"left\" width=\"100%\"  border=\"0\" cellpadding=\"0\" cellspacing=\"0\">";
			print "<tr><td class=\"yel_cell_top\">&nbsp;</td></tr> ";
			print "<tr><td class=\"yel_cell_center\" align=\"center\"><font class=\"small\">Please Select Valid Service Name & City name .......Thanks (last loop)</font></td></tr>";
			print "<tr><td class=\"yel_cell_bottom\">&nbsp;</td></tr> ";
			exit;
	}
